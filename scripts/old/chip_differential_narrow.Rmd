---
title: "chip differential narrow"
...
```{r}
path="count-tables/"
df = read.table(paste0(path,"Jan14th_narrow_counts.tsv"),sep="\t",header=T,row.names = 1)
head(df[,1:3])
...
design.matrix = design.matrix[colnames(df),]
dim(df)
broadpeaks = c("H3K27me3","H3K36me3","H3K9me3")
design.narrow = design.matrix[!design.matrix$histone %in% broadpeaks,]
design.broad = design.matrix[design.matrix$histone %in% broadpeaks,]
df = df[,row.names(design.narrow)]
dim(df)
```
```{r}
summary(colnames(df) == row.names(design.narrow))
```
...
```{r}
design.narrow$treatment = factor(design.narrow$treatment)
design.narrow$histone = factor(design.narrow$histone)
design.narrow$batch = factor(design.narrow$batch)
dim(design.narrow)
```

```{r}
levels(design.narrow$histone)
```


#write design matrix
```{r}
outputname = "narrowpeaks"
#write.table(df,file = "atac_rawcounts.tsv",sep = "\t",quote = F, col.names = NA)
write.table(design.matrix,file = paste0("design_matrix_hyp-norm_",outputname,".tsv"),sep = "\t", quote = F, col.names = NA)
```

# Granges for all regions
```{r}
#path="/home/master/WORK/atac/private/remi/aug14.2016_alldata/DESEQ2/bedfiles/"
bedfile="mergedpeaks/peaks_narrow_merged.bed"
totalwindows=create_granges_frombed3(paste0(bedfile))
length(totalwindows)
dim(df)
df_new <- df[ rowSums(df) > 15,  ]
paste0(nrow(df_new),"*",ncol(df_new))
totalwindows_new = create_grangesnew_frombed3(data.frame(totalwindows)[as.logical(rowSums(df) >15),])
nrow(df_new)
...
```{r}
dds  = DESeqDataSetFromMatrix(countData= df_new, colData= design.narrow, ~ treatment + histone + batch , rowRanges=totalwindows_new)
