...
if (length(args)!=2)
{
  stop("Usage: Rscript genowidereplicate-correlation.R <XMLconfig. file> <outputname>", call.=FALSE)
}
if (!file.exists(args[1]))
{
  stop("Input XML file not found!\
Usage: Rscript plotinsertsize.R <XML file> <outputname>", call.=FALSE)
}
#libraries
...
#heatmap
png(filename = paste0(outputname,"_heatmap.png"),width = 480, height = 480)
...
createscatterplot = function(x,y,namex,namey){
  png(filename =  paste0(namex,"-vs-",namey,"_correlation.png"))
  plot(x,y, col="magenta",pch=16, log="xy", main="correlation-plot" , xlab = namex, ylab= namey,xaxt ='n',yaxt = 'n')
  dev.off()
}

createscatterplot(df.filt[,1],df.filt[,2],bam.names[1],bam.names[2])
if(length(bam.names)>2)
{
  createscatterplot(df.filt[,1],df.filt[,3], bam.names[1],bam.names[3])
  createscatterplot(df.filt[,2],df.filt[,3],bam.names[2],bam.names[3])
}
#png(filename = paste0(bam.names[2],"vs",bam.names[3],"-correlation.png"),width = 480, height = 480)
#plot(df.filt[,2],df.filt[,3], col=magenta, 
#     pch=16, log="xy", main="correlation-plot" , xlab = bam.names[2], ylab= bam.names[3],
#     xaxt ='n',yaxt = 'n')
#dev.off()
