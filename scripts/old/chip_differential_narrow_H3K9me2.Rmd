---
title: "chip differential narrow h3k9me2"
...
```{r}
df = df[,grepl("H3K9me2",colnames(df)),]
exclude = c("H8_H3K9me2_E2",
            "N3_H3K9me2_E2",
            "N3_H3K9me2_E6",
            "N3_H3K9me2_E1",
...
dim(df)
df = df[,row.names(design.matrix)]
dim(design.matrix)
```


```{r}
summary(colnames(df) == row.names(design.matrix))
```


```{r}
design.matrix = design.matrix[colnames(df),]
design.matrix$treatment = factor(design.matrix$treatment)
design.matrix$histone = factor(design.matrix$histone)
design.matrix$batch = factor(design.matrix$batch)
design.matrix
```

```{r}
levels(design.matrix$histone)
```
...
dim(df)
cutoff = 100
df_new <- df[ rowSums(df) > cutoff,  ]
paste0(nrow(df_new),"*",ncol(df_new))
totalwindows_new = create_grangesnew_frombed3(data.frame(totalwindows)[as.logical(rowSums(df) >cutoff),])
nrow(df_new)
...
```{r}
dds  = DESeqDataSetFromMatrix(countData= df_new, colData= design.matrix, ~ treatment  + batch , rowRanges=totalwindows_new)
